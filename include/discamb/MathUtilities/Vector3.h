...
template <typename T>
template <typename INDEX_TYPE>
const T &Vector3<T>::operator()(INDEX_TYPE i) const {
...
template <typename T>
inline void Vector3<T>::get(T &_x, T &_y, T &_z) const {
    _x = x;
    _y = y;
    _z = z;
}
...
template <typename T>
inline T Vector3<T>::norm2() const {
    return x * x + y * y + z * z;
}
...
template <typename T>
inline const T operator*(const Vector3<T> &u, const Vector3<T> &v) {
...
template <typename T>
inline const Vector3<T> operator*(const T &scalar, const Vector3<T> &v) {
    return Vector3<T>(scalar * v.x, scalar * v.y, scalar * v.z);
}

template <typename T>
inline const Vector3<T> operator*(const Vector3<T> &v, const T &scalar) {
...
template <typename T>
inline const Vector3<T> cross_product(const Vector3<T> &u,
                                      const Vector3<T> &v) {
...
template <typename T>
std::istream &operator>>(std::istream &inp, Vector3<T> &v) {
    inp >> v.x >> v.y >> v.z;
    return inp;
}
