#ifndef _DISCAMB_SCATTERING_HansenCoppens_SF_Engine2_H_
#define _DISCAMB_SCATTERING_HansenCoppens_SF_Engine2_H_
...
class HansenCoppens_SF_Engine2
{
public:
    HansenCoppens_SF_Engine2();
    ~HansenCoppens_SF_Engine2();
...
private:
    ...
    */
    ...
    template<int l_max>
    std::complex<REAL> combine_multipolar_terms(
        const std::vector<std::vector<REAL> > &p_lm,
        const std::vector<REAL> &radial,
        std::vector<std::vector<REAL> > &sphericalHarmonics);
    ...
    std::complex<double> calculateDeformationValence(
        const std::vector<std::vector<REAL> >& p_lm, // coefficients for multipolar terms (with wavefunction normalization of spherical harmonics)
        const std::vector<REAL>& g_functions_and_slater_normalization,
        const Matrix3<REAL>& local_coordinates_system,
        int maxL,
...
void HansenCoppens_SF_Engine2::calculateSF(
    ...
    T &singleHklResultsProcessor)
{
    //cout << "calls HansenCoppens_SF_Engine2_6::calculateSF" << endl;
...
template<int l_max>
std::complex<REAL> HansenCoppens_SF_Engine2::combine_multipolar_terms(
    const std::vector<std::vector<REAL> > &p_lm,
    const std::vector<REAL> &radial,
    std::vector<std::vector<REAL> > &sphericalHarmonics)
{
...
#endif /*_DISCAMB_SCATTERING_HansenCoppens_SF_Engine2_H_*/
