#ifndef _DISCAMB_SCATTERING_HANSENCOPPENS_SF_ENGINE_H_
#define _DISCAMB_SCATTERING_HANSENCOPPENS_SF_ENGINE_H_
...
class HansenCoppens_SF_Engine
{
public:
    HansenCoppens_SF_Engine();
    ~HansenCoppens_SF_Engine();
...
void HansenCoppens_SF_Engine::calculateSF(
    ...
    T &singleHklResultsProcessor)
{
    //cout << "calls HansenCoppens_SF_engine_6::calculateSF" << endl;
...
template<int l_max>
std::complex<REAL> HansenCoppens_SF_Engine::combine_multipolar_terms(
    const std::vector<std::vector<REAL> > &p_lm,
    const std::vector<REAL> &radial,
    std::vector<std::vector<REAL> > &sphericalHarmonics)
{
...
#endif /*_DISCAMB_SCATTERING_HANSENCOPPENS_SF_ENGINE_H_*/
