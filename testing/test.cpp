...
int main() {
  try {
    ...
    std::vector<discamb::sf_engine_data_types::HC_TypeParam> type_parameters;
    ...
    load_from_file("local_coordinate_systems.bin", local_coordinate_systems);
    ...
    std::vector<discamb::TargetFunctionAtomicParamDerivatives> d_target_dparam;
    ...
    load_from_file("include_atom_contribution.bin", include_atom_contribution);
    ...
    engine.calculateSF(
        unit_cell, wfn_parameters, type_parameters, atom_to_type_map,
        atom_to_type_map, atomic_positions, atomic_displacement_parameters,
        atomic_occupancy, anomalous_dispersion, atomic_multiplicity_factor,
        local_coordinate_systems, symmetry_operations, centrosymmetric,
        inversion_translation, h_vectors, hkl_indices, f, d_target_dparam,
        d_target_df, include_atom_contribution, n_threads, derivatives_switch,
        electron, atomic_number);
